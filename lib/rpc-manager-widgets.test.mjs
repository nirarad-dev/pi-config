...
test("reports when an extension is waiting for user input", async () => {
  const { context, events, wrapper } = createContext();
  const confirmation = context.confirm("Continue?", "Approve the next step");
  const request = events.find((event) => event.method === "confirm");

  assert.equal((await wrapper.send({ type: "get_state" })).needsUserInput, true);
  await wrapper.send({ type: "extension_ui_response", id: request.id, confirmed: true });
  assert.equal(await confirmation, true);
  assert.equal((await wrapper.send({ type: "get_state" })).needsUserInput, false);
  wrapper.destroy();
});
