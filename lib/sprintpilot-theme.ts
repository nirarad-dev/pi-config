 ...
 * sixteen ANSI slots. It carries no syntax-token or diff mappings, so the roles
 * below assign the palette's own colours using the conventional terminal
 * mapping — blue for keywords, green for strings, purple for numeric literals,
 * yellow for types, cyan for callables — and the diff colours reuse the
 * palette's green and red rather than introducing hues it does not contain.
 *
 ...
 *   - Ordinary code uses ANSI 7 (#d8dee9) rather than the theme's #ffffff
...
export const cursorDarkPrismTheme: Record<string, React.CSSProperties> = {
  ...
  number: token(CURSOR_DARK.magenta),
  boolean: token(CURSOR_DARK.magenta),
  constant: token(CURSOR_DARK.magenta),
  symbol: token(CURSOR_DARK.magenta),

  "class-name": token(CURSOR_DARK.yellow),
