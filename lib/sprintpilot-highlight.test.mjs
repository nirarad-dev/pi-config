...
test("the inlined Python token colours stay in step with the theme palette", () => {
  ...
  // The three tones sampled from Cursor's own rendering.
  assert.equal(palette.purple, "#a7a0f4");
  assert.equal(palette.yellow, "#e5c993");
  assert.equal(palette.text, "#d6d6dd");
  assert.ok(used.has(palette.cyan) && used.has(palette.purple) && used.has(palette.text));
});
