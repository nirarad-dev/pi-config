...
const instrumentationClientSource = await readFile(new URL("../instrumentation-client.ts", import.meta.url), "utf8");
...
test("loads pre-hydration scripts through the Next Script component", () => {
  assert.match(layoutSource, /import Script from "next\/script"/);
  assert.match(layoutSource, /id="theme-initializer" strategy="beforeInteractive"/);
  assert.doesNotMatch(layoutSource, /<script\b/);
});

test("filters the injected MetaMask initialization failure before hydration", () => {
  assert.match(instrumentationClientSource, /MetaMask: Failed to get initial state/);
  assert.match(instrumentationClientSource, /queueMicrotask\(installConsoleGuard\)/);
  assert.match(instrumentationClientSource, /setTimeout\(installConsoleGuard, 0\)/);
  assert.match(instrumentationClientSource, /Object\.defineProperty\(window\.console, "error"/);
  assert.match(instrumentationClientSource, /isMetaMaskInitializationFailure\(\[event\.reason\]\)/);
  assert.match(instrumentationClientSource, /event\.filename\.startsWith\("chrome-extension:\/\/"\)/);
});
